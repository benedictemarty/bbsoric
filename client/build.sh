...
# term.s + récepteur XMODEM + sauvegarde Sedoric/LOCI + police BBS concaténés.
SRC="$(mktemp --suffix=.s)"
trap 'rm -f "$SRC"' EXIT
cat "$HERE/term.s" "$HERE/xmodem.s" "$HERE/sedoric.s" "$HERE/loci.s" "$HERE/hires.s" "$HERE/altcharset.s" > "$SRC"
