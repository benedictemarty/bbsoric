...
| Symptôme | Pistes |
|----------|--------|
| Le menu modem ne répond pas / écran figé | mauvaise base ACIA → essayer l'autre choix (`1`/`2`) ; vérifier la base réelle de la carte. **Émulateur LOCI :** le modem (picowifi) du vrai LOCI est une ACIA à **`$0380`**, pas `$03A0`. Lancer `--loci --serial picowifi` (sans `--acia-addr`) et adresser **`$0380`** ; ne PAS forcer `--acia-addr 03A0` (= espace MIA → masque l'ACIA + fige le clavier). Cf. `phosphoric-findings.md` (F1). |
