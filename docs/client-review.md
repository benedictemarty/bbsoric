...
| 5c | 🔴 | **`input_line` ate every typed character** (regression from 5b): the normal-char path **fell through** into the backspace handler `il_back` (it stored + echoed the char, then immediately decremented `INLEN` and erased it). Net effect: **manual host/port entry was impossible** — nothing could ever be typed (the field stayed empty; only phonebook dialing worked, which hid the bug). | Added the missing `jmp il_skip` after the echo so an accepted character no longer falls into `il_back` (`term.s input_line`). Validated in `oric1-emu`: manual entry `127.0.0.1` / `6502` now reaches the protocol prompt, dials the local BBS and renders the grid (`scripts/test-emulateur-grille.sh`). |
...
| 12 | 🟡 | Weak client test coverage (`test-emulateur.sh` smoke test fragile, based on cycle counter) | **Partly addressed**: `scripts/test-emulateur-grille.sh` drives the *full* multi-step path (modem menu → manual entry → dial → BBS grid) deterministically via `--type-keys`. Key learning: `input_line` consumes one key + `wait_release` per character, and `--type-keys` *holds* a key until the next event — so each char (and notably the Enter) needs a `\p` pause to release. Remaining smoke fragility (cycle-count timing) stays in the backlog. |
